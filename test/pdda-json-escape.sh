...
out="$(env -i PATH=/usr/bin:/bin bash -c '. "$1"; pdda_json_escape "$2"' _ "$LIB" 'a "quoted" \path\ line
tab	end')"
assert_eq "$out" 'a \"quoted\" \\path\\ line\ntab\tend' \
  "node-absent fallback matches the node-present output for quote/backslash/newline/tab"
...
out="$(env -i PATH=/usr/bin:/bin bash -c '. "$1"; pdda_json_escape "$2"' _ "$LIB" "$raw")"
assert_eq "$out" 'a\u0001\u0002\bb\u001fc' \
  "node-absent fallback escapes arbitrary C0 control bytes as backslash-u00XX (and backslash-b via its short escape)"
decoded="$(round_trip "$out")"
