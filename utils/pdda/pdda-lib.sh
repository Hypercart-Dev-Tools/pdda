...
# Release lifecycle bucket (outside the 1-4 tree — cross-cutting shipping artifact, not a work item).
PDDA_RELEASES_DIR="${PDDA_RELEASES_DIR:-$PDDA_REPO_ROOT/PROJECT/releases}"
...
# Cached GitHub release-state file (one tag per line, '#'-comment lines ignored). Written by
# pdda-gh-release-sync.sh; read by `pdda.sh release-readiness` when gh is absent/offline.
PDDA_GH_RELEASE_CACHE="${PDDA_GH_RELEASE_CACHE:-$PDDA_REPO_ROOT/.pdda-gh-release-state.tsv}"
...
# --- GitHub release-state fetch (shared by `pdda.sh release-readiness` and pdda-gh-release-sync.sh) -
# Parallel to the issue-state helpers above; same cache producer/consumer contract, same offline-
# degrade pattern. Format: one release tag per line (no tabs — tags contain no whitespace).

# Live release tag list from gh. Non-zero (and empty) when gh fails.
_pdda_gh_release_table() {
  local slug
  slug="$(_pdda_gh_repo_slug)"
  if [ -n "$slug" ]; then
    gh release list -R "$slug" --limit 100 --json tagName \
...
# Write the release tag list to the gh-release cache, atomically. Returns non-zero on write failure.
pdda_write_gh_release_cache() {  # <table>
  local table="$1" tmp="$PDDA_GH_RELEASE_CACHE.tmp.$$"
  {
    printf '# pdda gh-release-state cache — regenerated by pdda-gh-release-sync.sh; do not edit by hand\n'
    printf '# generated: %s\n' "$(pdda_now_iso)"
    printf '# format: one GitHub release tag per line\n'
    [ -n "$table" ] && printf '%s\n' "$table"
  } > "$tmp" 2>/dev/null || { rm -f "$tmp"; return 1; }
  mv "$tmp" "$PDDA_GH_RELEASE_CACHE" 2>/dev/null || { rm -f "$tmp"; return 1; }
}

# --- Release doc helpers -----------------------------------------------------------------------

# Parse a YAML block-sequence field from a doc's frontmatter and print one item per line. Handles the
# canonical multi-value form only (block sequence, not inline [a, b]):
#   key:
#     - item1
#     - item2
# Items are trimmed. Returns nothing (silently) for a missing or empty list.
pdda_frontmatter_list_values() {  # <file> <key>
  pdda_frontmatter_lines "$1" | awk -v key="$2" '
    $0 ~ ("^" key ":") { in_key=1; next }
    in_key && /^[[:space:]]+-[[:space:]]/ {
      val=$0
      sub(/^[[:space:]]*-[[:space:]]/, "", val)
...
# List active (Draft or RC) release docs for roadmap-coverage. Never used in single-file-lint mode
# (releases are not in the PDDA_ONLY_FILE scoping paths).
pdda_list_release_active_docs() {
  local dir="${PDDA_RELEASES_DIR:-$PDDA_REPO_ROOT/PROJECT/releases}"
  [ -d "$dir" ] || return 0
  local file status_raw status_lc
